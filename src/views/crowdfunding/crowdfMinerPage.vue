...
import * as $crowdfMinerPageApi from "@/api/member/crowdfMinerPage";
...
const pageData: any = reactive({
  ...
  tableParams: {
    columns: [
      { label: "编号", prop: "minerNo", width: "180px" },
      { label: "状态", prop: "status", width: "180px", slot: "statusScope" },
      { label: "开始时间", prop: "startTime", width: "180px", slot: "startTimeScope" },
      { label: "最大产值", prop: "amount", width: "180px", slot: "amountScope" },
      { label: "已收益", prop: "releaseAmount", width: "180px", slot: "releaseAmountScope" },
      { label: "待领取收益", prop: "releaseReward", width: "180px", slot: "releaseRewardScope" }
    ],
    list: [],
    loading: false,
    pagination: {
...
<template>
  <el-card :shadow="'never'">
    ...
    <pure-table :data="pageData.tableParams.list" :columns="pageData.tableParams.columns" row-key="id" border stripe
      ...
      <template #releaseAmountScope="scope">
        <span>{{ fromWei(scope.row[scope.column.property]) }}</span>
      </template>

      <template #releaseRewardScope="scope">
        <span>{{ fromWei(scope.row[scope.column.property]) }}</span>
      </template>
